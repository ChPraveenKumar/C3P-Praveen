								    	   ...
								    	   if ($rootScope.modifyScheduler === true){
											...
											dataFactory.postDataFact(uploadUrl, Data, transformRequest, headers)
								               .then(function(response){
												   ...
												   window.open('http://localhost:4200/requestdetails?dashboardReq_Details='+requestId+'&version='+version+'&page=Configuration','_self');
								               }, function (error) {
								            	   alertPopUp('Error', 'Unable to load data');
								            });
								       }else{
					...
					dataFactory.postDataFact(uploadUrl, Data, transformRequest, headers)
		               .then(function(response){
		            	   ...
		            	   if (response.data.entity.status == true){
		            	   		if (response.data.entity.templateId){
									$scope.configuartion.templateId = response.data.entity.templateId;
								...
								$http(
									...
									bootbox.confirm({
										...
										callback : function() {
											...
											//window.open('http://localhost:4200/requestdetails?dashboardReq_Details='+response.data.requestId+'&version='+response.data.version+'&page=Configuration','_self');
										}
									});
									//console.log(response)
