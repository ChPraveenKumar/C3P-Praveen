...
public class ShowMemoryTest {
	...
	public String memoryInfo(String ip, String username, String password,String routername, String region,String type) throws IOException
	{
		String result=null;
		ShowMemoryTest.loadProperties();
		String port = ShowMemoryTest.PROPERTIES
				.getProperty("portSSH");
		try {
			JSch jsch = new JSch();
			...
			result=printMemoryInfo(input, channel,
					routername,
					region,type);
			channel.disconnect();
			session.disconnect();
	...
	public Double getMemoryUsed(String hostname, String region,String type)
	{
		...
		try {
			...
			{
			filepath = ShowMemoryTest.PROPERTIES
					.getProperty("responseDownloadPathHealthCheckFolder")
					+ "//"+"Pre_"+ hostname
					+ "_" + region + "_MemoryInfo.txt";
			}
			else
			{
				filepath = ShowMemoryTest.PROPERTIES
	...
	public String printMemoryInfo(InputStream input, Channel channel,
