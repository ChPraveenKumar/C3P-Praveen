...
public class XMLToJSONConverter {
	...
	public JSONObject getDataForOrder(String orderID) {
		RequestInfoSO result = new RequestInfoSO();
		...
		try {
			 ...
			 /*JSONObject objjj=xmlJSONObj.getJSONObject("RequestList");
			 JSONArray array=objjj.getJSONArray("Request");
			 InternetLcVrfSO internetLvcrfSo=new InternetLcVrfSO();
			 ...
			 for(int i=0; i<array.length();i++)
			 {
				 ...
				 if(orderid.equalsIgnoreCase(orderID))
				 {
					 JSONObject customerInfoObject=arrayObj.getJSONObject("customerInfo");
					 {
						 ...
						 //result.setNetWorkType(customerInfoObject.getString("networkType")); hardcoded on UI 
					 }
					 JSONObject deviceInfoObject=arrayObj.getJSONObject("deviceInfo");
					 {
					...
					JSONObject interfaceRoutingInfoObj=arrayObj.getJSONObject("interfaceRoutingInfo");
					{
	...
	public JSONObject getDataForOrder(String ip,String zip) {
		...
		try {
			...
			String filePath = XMLToJSONConverter.PROPERTIES
					.getProperty("configurationFilesXML");
			...
			{
			//This case will not come as user will input only either of parameters at a time
			}
			else if(ip.isEmpty())
			{
