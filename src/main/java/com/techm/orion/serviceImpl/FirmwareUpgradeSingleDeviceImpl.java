...
@Service
public class FirmwareUpgradeSingleDeviceImpl implements
		FirmwareUpgradeSingleDeviceInterface {
	...
	/*
	 * Add record for single firmupgrade Device with Below validations will be applicable 
	 * i->Duplicate Image_filename: For same Vendor and Family, if new image filename exactly matches with any of existing Image_filename, 
	 * then return with error message : Image already exists 
	 * ii->Duplicate Display Name : For same Vendor and Family, if new Display Name exactly matches with any of existing Display Name, 
	 * then return with error message : Display Name already exists
	 * iii->Lower Image : For same Vendor and Family, if new Display Name is less than the highest Display Name available, 
	 * then return with error message : A higher version already exists. Are you sure to add you want to add this version ?
	 */
	@Override
	public Map<String, String> addFirmWare(String firmwareData)
			throws Exception {
		...
		try {
			...
			String checkHigherDisplayNameExist = firmupgradeRepository
					.checkHigherDisplayName(vendor, family);
			...
			if (checkHigherDisplayNameExist != null
					&& checkHigherDisplayNameExist.toString()
							.compareToIgnoreCase(displayName.toString()) > 1)
	...
	/*
	 * Add record for single firmupgrade Device with Below validations will be applicable 
	 * If user confirms (from UI) , that he wants to add the lower image, then it should get added in the database table. 
	 */
	@Override
	public Map<String, String> addFirmWareLowerImage(String firmwareData)
			throws Exception {
	...
	/*
	 *  When user upgrades a Device from UI, this API will provide value for field “Update OS” by searching for
	 *  highest available OS version (from column Display Name) against the selected device’s vendor and device model. 
	 */
	@Override
	public Map<String, String> findHighestOsVersion(String firmwareData)
			throws Exception {
		...
		try {
			...
			String checkHigherOsVersion = firmupgradeRepository
					.checkHigherOsVersion(vendor, family);

			if (checkHigherOsVersion != null
					&& checkHigherOsVersion.toString().compareToIgnoreCase(
							osversion.toString()) >= 1)
