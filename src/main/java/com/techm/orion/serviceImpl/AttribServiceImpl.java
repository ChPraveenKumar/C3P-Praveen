...
public class AttribServiceImpl implements AttribSevice{
	...
	public List<PredefinedMappedAtrribPojo> getAllMasterPredefinedGenericAtrribData() {
		List<PredefinedGenericMasterAttribEntity> predefinedGenericTemplateAtrribList = predefinedGenericMasterAtrribDao.findAll();
	...
	public GetAttribResponseEntity createGetAttribResponse(List<PredefinedMappedAtrribPojo> predefinedGenericAtrribList, List<AttribUIComponentPojo> attribUIComponentList,List<AttribValidationPojo> attribValidationList, List<CategoryMasterPojo> masterCategoryList){
        ...
        List<PredefinedAtrribPojo> predefinedAttribList = predefinedGenericAtrribList.stream()
        		.filter(predefinedList -> predefinedList.getType().equals("PREDEFINED"))
        		.map(predefined -> {        			
	...
	public List<PredefinedMappedAtrribPojo> getAllTemplatePredefinedGenericAtrribData(String templateId) {
		List<PredefinedGenericTemplateAttribEntity> predefinedGenericTemplateAtrribList = predefinedGenericTemplateAtrribDao.findAll();
		...
		if(!oldAttribList.isEmpty()) {
			predefinedGenericTemplateAtrribList = getRemainingAttrib(oldAttribList,predefinedGenericTemplateAtrribList);
		}
	...
	public List<PredefinedGenericTemplateAttribEntity> getRemainingAttrib(List<MasterAttributes> oldAttribList, List<PredefinedGenericTemplateAttribEntity> predefinedGenericTemplateAtrribList) {
		/*to filter used attrib from all attribs*/
		Predicate<PredefinedGenericTemplateAttribEntity> notInList = pgtaEntity -> ! oldAttribList.stream().anyMatch(mAttrib -> (!pgtaEntity.getAttribType().equals("PREDEFINED")&&(pgtaEntity.getAttribName().equals(mAttrib.getName()))));
		List<PredefinedGenericTemplateAttribEntity> newList = predefinedGenericTemplateAtrribList.stream().filter(notInList).collect(Collectors.toList());
