...
import com.techm.orion.pojo.RequestVersionCountPojo;
...
public class GetAllRequestDashboardViewService implements Observer {
	  ...
	  //  elapsedtimings = new ArrayList<ElapsedTimeFormatPojo>();
	   ...
	   /* List<RequestInfoSO>countedObject=new ArrayList<RequestInfoSO>();
	    ...
	    if(finalList.size()>0)
	    {
	    	...
	    	{
	    		if(comapreObj.getDisplay_request_id().equalsIgnoreCase(finalList.get(j).getDisplay_request_id()))
	    		{
	    			isPresent=true;
	    			break;
	    		}
	    	...
	    	if(isPresent == false)
	    	{
	    	    ...
	    	    {
	    	    	if(comapreObj.getDisplay_request_id().equalsIgnoreCase(detailsList.get(k).getDisplay_request_id()))
	    	    	{
	    ...
	    obj.put("MaxElapsedTime",
	    		dcmConfigService.getMaxElapsedTime(finalList));
	...
	public Response getListForConfigurationFeature(@RequestBody String templateFeatureRequest){
		...
		try{	
		    ...
		    List<String> featureList=dcmConfigService.getConfigurationFeature(configReqToSendToC3pCode.getRegion(), configReqToSendToC3pCode.getVendor(), configReqToSendToC3pCode.getModel(), configReqToSendToC3pCode.getOs(), configReqToSendToC3pCode.getOsVersion());
