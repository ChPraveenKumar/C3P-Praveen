...
public class GetConfigurationTemplateForModify {
	...
	public JSONObject createConfigurationTemplateModify(@RequestBody String configRequest){
		...
		try{	
		    ...
		    if(internetLvcrf.containsKey("neighbor1_remoteAS")&&  internetLvcrf.get("neighbor1_remoteAS").toString()!="")
		    {
		    createConfigRequest.setNeighbor1_remoteAS(internetLvcrf.get("neighbor1_remoteAS").toString().toUpperCase());
		    }
		    if(internetLvcrf.containsKey("neighbor2_remoteAS") &&  internetLvcrf.get("neighbor2_remoteAS").toString()!="")
		    {
		    	createConfigRequest.setNeighbor2_remoteAS(internetLvcrf.get("neighbor2_remoteAS").toString().toUpperCase());
		    }
		    else
		    {
		    ...
		    if(json.containsKey("snmpHostAddress"))
			  ...
			  if(json.containsKey("lanInterface"))
				...
				createConfigRequest.setDisplay_request_id(json.get(
						"display_request_id").toString());
				createConfigRequest.setRequest_version(Double.parseDouble(json
						.get("request_version").toString()));
				createConfigRequest
						.setRequest_parent_version(Double.parseDouble(json.get(
								"request_parent_version").toString()));

				createConfigRequest.setRequestId(createConfigRequest.getDisplay_request_id());
				DecimalFormat numberFormat = new DecimalFormat("#.0");
				createConfigRequest.setRequest_version(Double
						.parseDouble(numberFormat.format(createConfigRequest
