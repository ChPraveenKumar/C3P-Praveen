...
public class GetIPfromEIPAM implements Observer {
	...
	try {
	    Gson gson = new Gson();
	    EIPAMEntity dto = gson.fromJson(searchParameters,
	    		EIPAMEntity.class);
	    site = dto.getSite();
		...
		try {
			...
			if(!site.isEmpty() && !customer.isEmpty() && !service.isEmpty()&& !region.isEmpty() )
				
				{
					...
					{
						if(detailsList.get(i).getSite().equals(site) && detailsList.get(i).getCustomer().equals(customer) && detailsList.get(i).getService().equals(service) && detailsList.get(i).getRegion().equals(region) )
						{  
							
							detail = eipamEntityRepository.findBySiteAndCustomerAndServiceAndRegion(site,customer,service, region);
