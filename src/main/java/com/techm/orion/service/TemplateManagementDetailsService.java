...
public class TemplateManagementDetailsService {
	...
	public boolean addNewFeature(String comand_display_feature,String command_to_add,String command_type,String templateId,int parentid,int save,int topLineNum,int bottomLineNum,boolean dragged,int hasParent,String newFeature,String version,String lstCmdId)
	{
		...
		else
		{
			...
			result=templateManagementDao.createTemperorySequence(templateId,comand_display_feature,command_to_add,command_type,parentid,topLineNum,bottomLineNum,dragged,hasParent,newFeature,lstCmdId);
	...
	public List<GetTemplateMngmntActiveDataPojo> getDataForLeftPanel(String templateId,String tempKey,String currentTemplateId,boolean flag) throws Exception
	{
	...
	public List<GetTemplateMngmntPojo> getCommandForActivefeatures(String templateId) throws Exception
	{
		...
		for (Iterator iterator = templatecommandList.iterator(); iterator
				.hasNext();) {
			...
			getTemplateMngmntPojo.setConfName(getTemplateMngmntActiveDataPojo.getChildKeyValue().replaceAll("\\s{1,}",""));
	...
	public List<GetTemplateMngmntPojo> getCommandsforselectedchildfeatures (JSONArray nameArrray,JSONArray checkedArray,String templateid) throws Exception
	{
	TemplateManagementDao templateManagementDao=new TemplateManagementDao();
	GetTemplateMngmntPojo getTemplateMngmntPojo=null;
	List<GetTemplateMngmntActiveDataPojo> templatecommandList=new ArrayList<GetTemplateMngmntActiveDataPojo>();
	...
	public Map<String, String> addTemplate(String vendor, String deviceType,String model,String os,String osVersion,String region,String templateId)
	{
		Map<String,String> result=new HashMap<String, String>();
		TemplateManagementDao templateManagementDao=new TemplateManagementDao();
		result=templateManagementDao.createTemplateBasicConfig(vendor, deviceType,model, os,osVersion,region,templateId);
		return result;
	}
	...
	public String replacePlaceHolders(List<String>featureList,String template)
	{
		...
		if(isLanSelected)
		{
			...
			editedtemplate=template.replace("interface <lanInterface>\ndescription desc\nLAN ip address <lanIp> <lanMaskAddress>\nno shutdown\nnegotiation auto\n", LanExpandedBlock);
		}
		if(isWanSelected)
		{
			...
			{
			editedtemplate=editedtemplate.replace("interface <name>\ndescription <description>\nip address <ip> \n<mask>\nbandwidth <bandwidth>\nencapsulation <encapsulation>\n ", wanExpandedblock);
		
			}
			else
			{
			editedtemplate=template.replace("interface <name>\ndescription <description>\nip address <ip> \n<mask>\nbandwidth <bandwidth>\nencapsulation <encapsulation>\n ", wanExpandedblock);
	...
	public boolean savenewfeatureinCommandList(String parent,String commandName,String commandValue,String templateID)
	{
