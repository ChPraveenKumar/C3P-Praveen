	...
	public RequestInfoCreateConfig setEntityToPojo(RequestInfoEntity entity) {
		...
		if (entity != null) {
			...
			if (entity.getStatus().equalsIgnoreCase("Success") || entity.getStatus().equalsIgnoreCase("success")) {
				...
				Timestamp endDateOfProcessing = entity.getEndDateOfProcessing();
				Date d1_d = null;
				Date d2_d = null;

				try {
					d1_d = format.parse((covnertTStoString(dateOfProcession)));
					d2_d = format.parse((covnertTStoString(endDateOfProcessing)));
