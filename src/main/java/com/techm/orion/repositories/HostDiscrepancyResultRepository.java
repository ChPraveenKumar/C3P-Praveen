...
public interface HostDiscrepancyResultRepository extends JpaRepository<HostDiscrepancyResultEntity, Long> {
	...
	@Query(value = "SELECT distinct hid_discrepancy_flag FROM c3p_t_host_inv_discrepancy where hid_discrepancy_flag between '1' and '3' and device_id =:deviceId and hid_resolved_flag ='N'", nativeQuery = true)
	Set<String> findHostDiscrepancyValue(@Param("deviceId") String deviceId);

	@Query(value = "SELECT * FROM c3p_t_host_inv_discrepancy where hid_discrepancy_flag between '1' and '3' and device_id=:deviceId and hid_resolved_flag='N' and hid_discovery_id=:discovryId and hid_in_scope ='Y'", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM c3p_t_host_inv_discrepancy where device_id =:deviceId And hid_ip_address =:ipAddress", nativeQuery = true)
	List<HostDiscrepancyResultEntity> findListOfDeviceHostDiscrepancy(@Param("deviceId") String deviceId,
			@Param("ipAddress") String ipAddress);
