...
public interface CustomerStagingImportRepo extends JpaRepository<CustomerStagingEntity, Integer> {
	...
	@Query(value = "SELECT distinct model  FROM requestinfo.t_tpmgmt_glblist_m_models", nativeQuery = true)
	List<String> findModel();

	@Query(value = "SELECT vendor FROM requestinfo.t_tpmgmt_glblist_m_vendor", nativeQuery = true)
	List<String> findSupportedVendor();

	@Query(value = "SELECT distinct os FROM requestinfo.t_tpmgmt_glblist_m_os", nativeQuery = true)
	List<String> findOS();

	@Query(value = "SELECT distinct osversion FROM requestinfo.t_tpmgmt_glblist_m_osversion", nativeQuery = true)
