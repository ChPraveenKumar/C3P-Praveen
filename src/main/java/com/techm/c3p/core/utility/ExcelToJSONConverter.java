...
import com.techm.c3p.core.pojo.ExcelToJSONConfigPojo;
...
public class ExcelToJSONConverter {
	...
	// private static final String FILE_NAME = "/resources/standard_variable_in
	// _ipdp.xlsx";
	...
	public JSONObject convert(MultipartFile file) {
		...
		try {
			 ...
			 * 
			 * for (int i = wb.getNumberOfSheets() - 1; i >= 0; i--) { XSSFSheet tmpSheet =
			 * (XSSFSheet) wb.getSheetAt(i); if (tmpSheet.getSheetName().equals(sheetName))
			 * { wb.removeSheetAt(i); sheetOfParams=tmpSheet; } //logger.info("workbook: " +
			 * wb);
			 * 
			 * }
			 */
			Sheet sheet = wb.getSheetAt(2);
			logger.info("worksheet: " + sheet);
			Row row;
			...
			while (iterator.hasNext()) {
				...
				if (nextRow.getRowNum() == 9 || nextRow.getRowNum() == 10 || nextRow.getRowNum() == 11
						|| nextRow.getRowNum() == 12) {
					...
					pojo = new ExcelToJSONConfigPojo();
					pojo.setVariableName(new DataFormatter().formatCellValue(nextRow.getCell(1)));
					pojo.setVariable(new DataFormatter().formatCellValue(nextRow.getCell(1)));
					pojo.setVariableValue(new DataFormatter().formatCellValue(nextRow.getCell(2)));
				...
				if (nextRow.getRowNum() >= 22) {
					...
					pojo = new ExcelToJSONConfigPojo();
					pojo.setVariableName(new DataFormatter().formatCellValue(nextRow.getCell(1)));
					pojo.setVariable(new DataFormatter().formatCellValue(nextRow.getCell(3)));
					pojo.setVariableValue(new DataFormatter().formatCellValue(nextRow.getCell(2)));
					productsList.add(pojo);
				}
		...
		try {
			...
			boolean interfaceRoutinginfoRoutingFlag = false, interfaceRoutinginfoWanIntFlag = false,
					interfaceRoutinginfoVpnFlag = false, interfaceRoutinginfoEnablePasswordFlag = false,
					interfaceRoutinginfoBannerFlag = false, interfaceRoutinginfoLoopbackInterfaceFlag = false,
					interfaceRoutinginfoSnmpFlag = false;
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if (pojoObj.getVariableName().equalsIgnoreCase("Router Type:")) {
					...
					if (Integer.parseInt(model) != 7200 && Integer.parseInt(model) != 1921
							&& Integer.parseInt(model) != 2901 && Integer.parseInt(model) != 3660) {
						hasError = true;
				 ...
				 * else if(pojoObj.getVariableName().equalsIgnoreCase("IP Address")) {
				 * lanInterfaceObject.put("lanInterfaceIP",pojoObj.getVariableValue());
				 * pojoObj.setVariableName("lanInterfaceIPAddress"); finalList.add(pojoObj); }
				 * else if(pojoObj.getVariableName().equalsIgnoreCase("Subnet Mask")) {
				 * lanInterfaceObject.put("lanInterfaceSubnetMask",pojoObj.getVariableValue());
				 * pojoObj.setVariableName("lanInterfaceSubnetMask"); finalList.add(pojoObj); }
				 */

				logger.info("JSON");
			}
			// Logic to get networkIP and subnet
			// mask.............................................................................................
			 ...
			 * mapper.writeValueAsString(finalList);
			 */
			logger.info("JSON");
			...
			dcmConfigService.addEIPAMRecord(IPAMcustomerName, IPAMsiteID, IPAMipAdd, IPAMmask, "MIS", "CANADA");
		} catch (Exception e) {
