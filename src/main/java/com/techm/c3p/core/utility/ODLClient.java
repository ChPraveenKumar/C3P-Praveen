...
public class ODLClient {
	...
	public Boolean performTest(TestDetail test, RequestInfoPojo requestinfo, String user, String pwd)
			throws IOException {
		StringBuilder sb = new StringBuilder();
		String output = null;
		Boolean result = false;
						...
						if (rules.get(i).getDataType().equalsIgnoreCase("Text")) {
							...
							try {
								...
								if (bre.readLine() == null) {
									...
									if (bre.readLine().contains("File exists")
											|| bre.readLine().contains("File exist")) {
										// result = true;
								...
								try {
									...
									if (out >= val1 && out <= val2) {
										...
										result = requestDetailsService.updateTestStrategeyConfigResultsTable(requestID,
												...
												requestinfo.getRequestVersion(),test.getTestSubCategory());
									}
								} catch (Exception e) {
									// fail the test
