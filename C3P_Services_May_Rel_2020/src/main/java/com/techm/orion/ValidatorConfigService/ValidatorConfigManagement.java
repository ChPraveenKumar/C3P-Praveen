	...
	public String validate(CreateConfigRequestDCM configRequest) throws Exception {
		...
		try {
			...
			if (!ipValidator.isValid(configRequest.getMask())) {
				result="Interface Ip "
						+ configRequest.getMask() + " for name : "
						+ configRequest.getName()
						+ " is not a valid IP Address";
				throw new InvalidValueException(result);
			}
			...
			if (!ipValidator.isValid(configRequest.getNetworkIp())) {
				result="LC Vrf NetworkIp "
						+ configRequest.getNetworkIp()
						+ " is not a valid IP Address";
				throw new InvalidValueException(result);
			}
			/*if (!ipValidator.isValid(configRequest.getRouterBgp65k())) {
				result="LC Vrf Router Bgp 65k Ip "
						+ configRequest.getRouterBgp65k()
						+ " is not a valid IP Address";
				throw new InvalidValueException(result);
			}
			if (!ipValidator.isValid(configRequest.getNeighbor1())) {
				result="LC Vrf Neighbour 1 Ip "
						+ configRequest.getNeighbor1()
						+ " is not a valid IP Address";
				throw new InvalidValueException(result);
			}
			if (configRequest.getAS() == null
					|| configRequest.getAS().equals("")) {
				result="LC Vrf Neighbour 1 Ip "
						+ configRequest.getNeighbor1()
						+ " is present but AS is not there.";
				throw new InvalidValueException(result);
			}*/
