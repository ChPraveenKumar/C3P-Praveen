!---------------------------------------------------------------------------------------------------------------------------------------------- 
! ***Customer Name: ${(configRequest.customer)!""}     Site ID: ${(configRequest.siteid)!""}                 Region: ${(configRequest.region)!""}      Service: ${(configRequest.service)!""}      Network Type:Legacy                               
...
! ***WAN Interface:${(configRequest.name)!""}
! ***Template used:T_001          Config created on:${(configRequest.requestCreatedOn)!""}  
!----------------------------------------------------------------------------------------------------------------------------------------------
...
buffers fastEthernet0/0 permanent 8192
!
!
<#if ((configRequest.vrfName)?exists && (configRequest.vrfName) != "")>
ip vrf ${(configRequest.vrfName)!""}
description TestVRF
rd ${(configRequest.neighbor1_remoteAS)!""}:700
  route-target export ${(configRequest.neighbor1_remoteAS)!""}:200
  route-target import ${(configRequest.neighbor1_remoteAS)!""}:100
!
  route-target export ${(configRequest.neighbor1_remoteAS)!""}:400
  route-target import ${(configRequest.neighbor1_remoteAS)!""}:300
exit
!
</#if>
...
<#if ((configRequest.lanDescription)?exists && (configRequest.lanDescription) != "")>	
...
bgp log-neighbor-changes
  ...
  neighbor ${(configRequest.neighbor1)!""} bgp-multihop 3
  neighbor ${(configRequest.neighbor1)!""} password 0 ${(configRequest.vrfName)!""}
...
!
ip nat inside source list MIS_LC_NAT_POOL interface Loopback14 vrf ${(configRequest.vrfName)!""} overload
!
ip access-list extended MIS_LC_NAT_POOL
permit ip 11.170.47.0 0.0.0.255 any
exit
